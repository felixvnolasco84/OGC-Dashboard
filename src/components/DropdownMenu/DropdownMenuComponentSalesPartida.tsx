...
import EditSalesPartidaForm from "../Forms/EditSalesPartidaForm";

interface DropdownMenuComponentSalesPartidaProps {
...
export default function DropdownMenuComponentSalesPartida({
  partida,
  level,
  rowData,
}: DropdownMenuComponentSalesPartidaProps) {
  const seeSalesPaymentDetailsModal = useSeeSalesPaymentDetailsModal();
  const aggregatedDetailsModal = useAggregatedDetailsModal();
  const navigate = useNavigate();

  // Query payments based on level with proper sales project filtering
  ...
  // Level 2 (nivel 3): Get payments by sales_partida_id (specific sub-partida)
  const level2Payments = useQuery(
    api.sales_pagos.getBySalesPartidaId,
    isRealPartida ? { sales_partida_id: partida._id } : "skip"
  );

  // Level 0 (nivel 1): Get all payments for this partida name in sales project
  const level0Payments = useQuery(
    api.sales_pagos.getByPartidaName,
    level === 0 && partida.nombre && partida.sales_proyecto
      ? {
        partida_name: partida.nombre,
        sales_proyecto_id: partida.sales_proyecto
      }
      : "skip"
  );

  // Level 1 (nivel 2): Get all payments for this familia within the partida in sales project
  const level1Payments = useQuery(
    api.sales_pagos.getByFamilia,
    level === 1 && partida.familia && partida.nombre && partida.sales_proyecto
      ? {
        partida_name: partida.nombre,
  ...
  // Query all sales partidas in this project with the same partida name and familia
  const allPartidas = useQuery(
    api.sales_partidas.getBySalesProject,
    level === 1 && partida.sales_proyecto ? { salesProjectId: partida.sales_proyecto } : "skip"
  );

  // Check if this familia has any sub-partidas
  const hasSubPartidas = level === 1 && allPartidas
    ? allPartidas.some(
        (p: Doc<"sales_partidas">) =>
  ...
  const getContextualLabels = () => {
    switch (level) {
      case 0:
        return {
          title: "Unidad",
          editTitle: `Editar Unidad: ${rowData.displayName}`,
          viewDetailsText: "Ver resumen de unidad",
        };
      case 1:
  ...
  //Navigate to detail page with react router
  const handleViewDetails = () => {
    ...
    if (shouldNavigateToDetails) {
      navigate(`/dashboard/sales-partidas/${partida._id}`);
    } else {
  ...
  const handleViewTransactions = () => {
    ...
    seeSalesPaymentDetailsModal.onOpen(paymentContext);
  };
