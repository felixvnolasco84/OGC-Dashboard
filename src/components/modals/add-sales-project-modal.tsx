...
import { FolderPlus, Upload, FileSpreadsheet } from "lucide-react";
import { Separator } from "@/components/ui/separator";
import { toast } from "sonner";

export default function AddSalesProjectModal() {
  const isOpen = useAddSalesProjectModal((state) => state.isOpen);
  const onClose = useAddSalesProjectModal((state) => state.onClose);
  const formData = useAddSalesProjectModal((state) => state.formData);
  const updateFormData = useAddSalesProjectModal((state) => state.updateFormData);
  ...
  const uploadProjectData = useMutation(api.sales_partidas.createSalesPartida);
  const createTransaction = useMutation(api.sales_transacciones.createSalesTransaction);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Proyecto de ventas creado exitosamente', {
        description: `Se creó el proyecto "${formData.nombre}" con ${partidas.length} partidas.`,
        duration: 5000,
      });

      updateFormData({ nombre: '', descripcion: '', excel: null, comision_porcentaje: 0 });
      setFile(null);
      setShowExcelUploader(false);
      onClose();
    } catch (error) {
      console.error("Error creating sales project:", error);

      toast.error('Error al crear el proyecto de ventas', {
      ...
      <SheetContent className="w-[800px] sm:max-w-[800px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            ...
            {showExcelUploader ? "Subir Excel del Proyecto de Ventas" : "Crear Nuevo Proyecto de Ventas"}
          </SheetTitle>
          <SheetDescription>
            {showExcelUploader
              ? "Sube el archivo Excel con los datos del presupuesto de ventas"
