...
export default function AddSalePaymentModal() {
    ...
    const allPartidas = useQuery(
        ...
        paymentContext ? { salesProjectId: paymentContext.projectId } : "skip"
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
                ...
                for (const familia of partida.familias) {
                    ...
                    if (familia.isDirect && !hasSubPartidas) {
                        if (familia.partida_id && familia.partida_id !== "" && familia.monto && familia.monto > 0) {
                            lineItems.push({
                                sales_partida_id: familia.partida_id as Id<"sales_partidas">,
                                monto: familia.monto,
                            });
                        }
                    } else {
                        ...
                        for (const subPartida of familia.subPartidas) {
                            if (subPartida.partida_id && subPartida.partida_id !== "" && subPartida.monto > 0) {
                                lineItems.push({
                                    sales_partida_id: subPartida.partida_id as Id<"sales_partidas">,
            ...
            const result = await createTransaction({
                sales_proyecto: paymentContext.projectId,
            ...
            toast.success("Transacción registrada", {
                description: `Se creó la transacción con ${lineItems.length} concepto(s) exitosamente.`,
            });
            onClose();
        } catch (error) {
