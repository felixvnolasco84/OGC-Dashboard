...
export default function UploadSalesProjectTransactionsModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorCount = 0;      
      const errors: string[] = [];
      const createdTransactionIds: Array<{ id: Id<"sales_transacciones">; factura: string }> = [];
      ...
      for (const txn of response.transactions) {
        try {
          ...
          const transaccionId = await createSalesTransaction({
          ...
          // Store transaction ID for later processing
          createdTransactionIds.push({
            id: transaccionId as Id<"sales_transacciones">,
            factura: txn.factura,
          });
      ...
      if (successCount > 0) {
        const description = errorCount > 0 || errors.length > 0 
          ? `${successCount} transacciones creadas con algunos errores`
