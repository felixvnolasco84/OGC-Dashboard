...
function HistoryValue({ value }: { value: string }) {
  ...
  return (
    <details>
      <summary className="cursor-pointer text-gray-600">Ver contenido</summary>
...
export function RfiHistoryTimeline({
  ...
  if (history.length === 0) {
    return (
      <div className="rounded-sm border border-dashed border-gray-200 p-8 text-center">
      ...
      {history.map((item) => {
        ...
        return (
          <li key={item._id} className="flex gap-3 py-4 first:pt-0 last:pb-0">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm bg-gray-100 text-gray-500">
            ...
            <div className="min-w-0 flex-1">
              ...
              {showValueChange && (
                <div className="mt-3 grid gap-2 text-xs text-gray-600 sm:grid-cols-[minmax(0,1fr)_16px_minmax(0,1fr)] sm:items-start">
                  <div className="rounded-sm bg-[#FBFBFB] px-3 py-2">
                    <span className="mb-1 block text-[10px] uppercase tracking-wide text-[#A5A5A0]">
                      Antes
                    </span>
                    <HistoryValue value={oldValue} />
                  </div>
                  <ArrowRight className="hidden h-4 w-4 text-gray-400 sm:mt-5 sm:block" />
                  <div className="rounded-sm bg-[#FBFBFB] px-3 py-2">
                    <span className="mb-1 block text-[10px] uppercase tracking-wide text-[#A5A5A0]">
                      Después
                    </span>
              ...
              {responsePreview && (
                <div className="mt-3 rounded-sm bg-[#FBFBFB] px-3 py-2 text-xs leading-5 text-gray-600">
                  ...
                  {item.attachments.map((attachment) =>
                    attachment.url ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center justify-between gap-3 rounded-sm border border-gray-200 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50"
                      >
                      ...
                      <div
                        key={attachment._id}
                        className="rounded-sm border border-gray-200 px-3 py-2 text-xs text-gray-500"
                      >
                        {attachment.nombre}
                      </div>
                    ),
