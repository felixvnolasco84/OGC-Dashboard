...
export default defineSchema({
  ...
  documentos: defineTable({
    ...
    client_id: v.optional(v.string()), // Stable client-generated ID for offline bitacora attachments
    ...
    .index("by_client_id", { fields: ["client_id"] }),
  ...
  bitacora: defineTable({
    ...
    // Metadata
    uploaded_at: v.number(), // Timestamp
    client_id: v.optional(v.string()), // Stable UUID generated by offline-capable clients
    revision: v.optional(v.number()), // Optimistic concurrency revision
    sync_version: v.optional(v.number()), // Monotonic project-level synchronization version
    updated_at: v.optional(v.number()),
    created_by: v.optional(v.string()), // Clerk subject
    updated_by: v.optional(v.string()), // Clerk subject
    deleted_at: v.optional(v.number()), // Tombstone for offline clients
  }).index("by_proyecto", { fields: ["proyecto"] })
    .index("by_partida_id", { fields: ["partida_id"] })
    ...
    .index("by_client_id", { fields: ["client_id"] })
    .index("by_proyecto_sync_version", { fields: ["proyecto", "sync_version"] }),

  // One monotonic synchronization counter per project. Convex retries
  // concurrent writes to this row transactionally, so versions stay ordered.
  bitacora_sync_state: defineTable({
    proyecto: v.id("desarrollos"),
    version: v.number(),
    updated_at: v.number(),
  }).index("by_proyecto", { fields: ["proyecto"] }),

  // Durable receipts make client outbox retries idempotent across reloads.
  bitacora_sync_operations: defineTable({
    operation_id: v.string(),
    client_id: v.string(),
    proyecto: v.id("desarrollos"),
    log_id: v.id("bitacora"),
    operation: v.string(),
    revision: v.number(),
    sync_version: v.number(),
    clerk_id: v.string(),
    created_at: v.number(),
  }).index("by_operation_id", { fields: ["operation_id"] })
    .index("by_proyecto_version", { fields: ["proyecto", "sync_version"] }),

  bitacora_sync_changes: defineTable({
    proyecto: v.id("desarrollos"),
    sync_version: v.number(),
    log_id: v.id("bitacora"),
    client_id: v.string(),
  ...
  bitacora_upload_reservations: defineTable({
    proyecto: v.id("desarrollos"),
    operation_id: v.string(),
    ...
    clerk_id: v.string(),
    created_at: v.number(),
  }).index("by_operation_attachment", { fields: ["operation_id", "attachment_id"] })
